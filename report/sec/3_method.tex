...
For DINO-based models, we experiment with unfreezing the last few transformer blocks together with the final normalization layer. In particular, for
DINOv2 we report results obtained by unfreezing the last three transformer layers and the normalization layer. For SAM, we fine-tune the neck component
while keeping the image encoder frozen. This design allows the model to adapt to correspondence-specific cues while preserving the general semantic
...
the distribution. We empirically observe that large window radii (\eg $r=5$ or $r=10$) degrade performance, and therefore use smaller values
...
Images are resized according to the backbone requirements: $518 \times 518$ for DINOv2, $512 \times 512$ for DINOv3, and $1024 \times 1024$ for SAM.
